...
typedef struct _rwlock_t
{
    ...
    sem_t writelock;
    sem_t wmutex;
    sem_t readTry; // Lock for readers trying to acquire the CS
    int readers;   // To maintain count of readers
    int writers;   // To maintain count of writers
} rwlock_t;
...
void *reader(void *arg)
{
    ...
    for (int i = 0; i < thread.n; i++)
    {
        ...
        sem_wait(&filemutex);
        // Open the file
        outfile.open("RW-log.txt", std::ios::out | std::ios::app);
        if (!outfile.is_open())
        {
        ...
        // Sleepign for sometime to simulate CS execution
        ...
        // Mainting the count of readers, and the time taken by them to acquire the lock
        ...
        // Sleeping to simulate the exit section
...
void *writer(void *arg)
{
    ...
    for (int i = 0; i < thread.n; i++)
    {
        ...
        // Simulating the execution of the CS
        double randCSTime = exponentialRandom(uCS);
        sleep(randCSTime);
...
int main()
{
    ...
    remove("RW-log.txt");       // Deleting the pre existing RW-log file
    remove("Average_time.txt"); // Deleting the pre existing Average_time file
    ...
    timeWriters = new double[numberWriters]; // Array to store the time of each writer call to the CS
    ...
    // Write to the file again
    outfile << "Printing the data for Writers Preference:" << endl;
